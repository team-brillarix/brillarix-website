...
import type { Innovator } from '@/types/innovator';

export default function TrustedByInnovatorsClient({ innovators }: { innovators: Innovator[] }) {
  const swiperRef = useRef<SwiperType | null>(null);
  const sectionRef = useRef<HTMLDivElement>(null);
  const observerRef = useRef<IntersectionObserver | null>(null);

  const duplicatedData = Array(4).fill(innovators).flat();
  ...
  return (
    <div ref={sectionRef}>
      <Section
        ...
        headingAlign="center"
      >
        <div className="absolute z-0 top-54 lg:top-46 pointer-events-none">
          <Image
        ...
        >
          <Swiper
            ...
            loop={true}
            loopAdditionalSlides={Math.max(innovators.length, 2)}
            speed={2000}
            allowTouchMove={true}
            grabCursor={true}
